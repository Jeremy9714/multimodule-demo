(function ($, window, document, undefined) {
  ...
  MySelect.prototype = {
    ...
    _initChoose: function () {
      ...
      if (checks) {
        ...
        $(list).each(function (i, val) {self.$optionList.find('.select-picker-options-list-item[data-val=' + val + ']' + ' .duihao').removeClass('duihao-nocheck').addClass('duihao-checked')})
        this._isAll()
      }
    },
    ...
    _getOptionDatas: function () {
      ...
      if (this.$element.data('init')) {
        var dataArr = null
        if (this._isJSON(this.$element.data('init'))) {result = JSON.parse(this.$element.data('init'))} else {
          dataArr = this.$element.data('init').toString().split(',')
          $(dataArr).each(function (i, val) {
            var map = {val: val || '', text: val || '',}
            result.push(map)
          })
        }
      } else {
        this.$options.each(function () {
          var _this = $(this)
          var map = {val: _this.val() || '', text: _this.text() || '',}
          result.push(map)
        })
      }
      if (this.options.isSort) {result.sort(this._sortText)}
      if (result.length > 0 && this.options.multiple) {
        var text = this.options.model ? 'All ' + this.options.model : 'All'
        result.unshift({val: 'ALL', text: text,})
      }
      return result
    },
    _sortText: function (a, b) {return a.text - b.text},
    _drawList: function () {
      var self = this
      ...
      if (self._getOptionDatas().length > 0) {
        $(self._getOptionDatas()).each(function () {
          ...
          if (this.val == 'ALL') {$tmp.addClass('chooseAll')}
          self.$optionList.append($tmp)
        })
      } else {self._showEmpty(true)}
    },
    _showList: function () {
    ...
    _search: function (keyword) {
      ...
      let self = this, selectorBuffer = [], groupIndex = -1, itemCount = 0,
        liArray = this.$optionList.find('.select-picker-options-list-item'), itemTotal = liArray.length
      var checkGroup = function (currentIndex, last) {
        if (itemCount >= (currentIndex - groupIndex - (last ? 0 : 1))) {selectorBuffer.push(self.$el + ' .select-picker-options-list .select-picker-options-list-item:nth-child(' + (groupIndex + 1) + ')')}
      ...
      liArray.each(function (index, item) {
        ...
        if (currentIndex >= (itemTotal - 1)) {checkGroup(currentIndex, true)}
      })
    ...
    _showEmpty: function (isShow) {
      ...
      if (isShow) {this.$emptyList.removeClass('hidden')}
    },
    _check: function (e) {
      let _this = $(e.currentTarget)
      if (_this.is('.chooseAll')) {if (_this.find('.duihao-nocheck').length > 0) {this._checkAll()} else {this._emptyAll()}} else {
        if (this.options.multiple) {
          if (_this.find('.duihao-nocheck').length > 0) {_this.find('.duihao').removeClass('duihao-nocheck').addClass('duihao-checked')} else {_this.find('.duihao').addClass('duihao-nocheck').removeClass('duihao-checked')}
          this._isAll()
        } else {
          if (_this.find('.duihao-nocheck').length > 0) {
            this._emptyAll()
            _this.find('.duihao').removeClass('duihao-nocheck').addClass('duihao-checked')
          } else {_this.find('.duihao').addClass('duihao-nocheck').removeClass('duihao-checked')}
    ...
    _isAll: function () {
      ...
      if ($left && $left.length == 1) {
        ...
        if ($parent.is('.chooseAll')) {$left.removeClass('duihao-nocheck').addClass('duihao-checked')} else {this.$optionList.find('.select-picker-options-list-item.chooseAll .duihao').addClass('duihao-nocheck').removeClass('duihao-checked')}
      }
    },
    _emptyAll: function () {this.$optionList.find('.select-picker-options-list-item .duihao').addClass('duihao-nocheck').removeClass('duihao-checked')},
    _checkAll: function () {this.$optionList.find('.select-picker-options-list-item .duihao').removeClass('duihao-nocheck').addClass('duihao-checked')},
    _getCheckedValue: function () {
      let checkedArr = [], checkedArrValue = [], isAll = false
      this.$optionList.find('.select-picker-options-list-item').each(function () {
        if ($(this).find('.duihao-checked').length > 0) {
          if ($(this).is('.chooseAll')) {isAll = true} else {
            let text = $.trim($(this).text()), val = $.trim($(this).data('val')) || null
            checkedArr.push(text)
            checkedArrValue.push(val)
          }
        }
      })
      return {texts: checkedArr, vals: checkedArrValue, isAll: isAll}
    },
  ...
  $.fn.mySelect = function (options) {
    ...
    args.shift()
    this.each(function () {
      var $this = $(this), data = $this.data('mySelect')
      if (data) $this.data('mySelect').remove()
      $this.data('mySelect', new MySelect(this, options))
    })
  }
  $.fn.mySelect.defaults = {showCancel: false, showSearch: false, multiple: false, isSort: true}
  ...
  $(document).click(function (event) {
    ...
    if (!_con2.is(event.target) && !_con.is(event.target) && _con.has(event.target).length === 0) {$('.select-picker-search').removeClass('open')}
  })
